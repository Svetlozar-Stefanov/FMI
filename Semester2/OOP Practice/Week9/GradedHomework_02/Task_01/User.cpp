...
char* User::Read(const Book& book)
{
	currentBook = book;
...
std::ofstream& operator<<(std::ofstream& file, const User& user)
{
	file << user.username << ' ';
	file << user.password << ' ';

	myVector<unsigned> read;
	read = user.readBooks;
	file << read.GetSize() << ' ';
	...
	myVector<unsigned> written;
	written = user.writtenBooks;
